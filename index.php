...
.content-wrapper {
    ...
    height: 150px; /* Ajusta  */
    object-fit: cover;
    display: block; /* hace que la imagen ocupe el espacio del contenedor */
    }
    .card-content {
    flex: 1;
    }
...
<main>
    <div class="background-image">
        ...
        <!-- boton flotante de mensaje -->
<a href="chatusuario.php" class="floating-button">
            ...
            <?php foreach ($mapas as $t): ?>
                <div class="card">                  
                    ...
                    <div class="card-buttons">
                        <a href="park-info.php?lat=<?= htmlspecialchars($t['latitud']) ?>&lng=<?= htmlspecialchars($t['longitud']) ?>&nombre=<?= urlencode($t['nombre']) ?>&latitud=<?= urlencode($t['latitud']) ?>&longitud=<?= urlencode($t['longitud']) ?>&precio=<?= urlencode($t['precio']) ?>&imagen=<?= urlencode($t['imagen']) ?>&horario=<?= urlencode($t['horario']) ?>&capacidad=<?= urlencode($t['capacidad']) ?>&descripcion=<?= urlencode($t['descripcion']) ?>" class="btn-rutas">Ruta</a>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
        ...
        <div class="why-shulton container mt-5">
            <div class="row">
                ...
                <div class="col-md-6 text-left">
                    ...
                    <a href="registrarse.php" class="btn-register" style="text-align: center;">Regístrate Gratis</a>
                </div>
            </div>
        </div>
    </main>
